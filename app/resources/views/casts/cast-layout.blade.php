...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased">
       <header class="header">
        ...
        <button class="side-menu-veri" onclick="hideSideMenu()" style="display: none;">&#9776;</button>
        ...
        <div class="sub-header-comp-xx" onclick="showMyProfile()">
            <p><img src="{{Auth::guard('customer')->user()->profile ? asset('storage/' . Auth::guard('customer')->user()->profile) : asset('assets/images/profile.png')}}" alt="My Profile"><span> {{Auth::guard('customer')->user()->full_name}}</span></p>
        </div>
       ...
       <!-- <button class="add-admin-e"><i class="fas fa-user-tie"></i> New admin</button>-->
        <button class="my-profile-e"><a href="/casts/my-profile/{{Auth::guard('customer')->user()->id}}"><i class="fa fa-user"></i> My profile</a></button>
...
@endif
            ...
            <form action="/messages" method="POST" class="message-sender-class">
                @csrf
                <input type="hidden" name="user_role" id="" value="3">
                <input type="hidden" name="profile" id="" value="{{Auth::guard('customer')->user()->profile}}">
                <input type="hidden" name="sender_name" id="" value="{{Auth::guard('customer')->user()->full_name}}">
